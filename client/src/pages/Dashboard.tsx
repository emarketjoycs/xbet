...
import { Wallet, TrendingUp, Trophy, Clock, ArrowUpRight, ArrowDownLeft } from 'lucide-react';
...
export default function Dashboard() {
  ...
  // INTEGRAÇÃO COM BACKEND: Descomentar quando hook estiver pronto
  // const { getUserBalance, getUserBets, claimWinnings } = useBettingContract();
  ...
  // Dados mockados - SUBSTITUIR COM DADOS REAIS DO BACKEND
  const userBalance = {
    total: 1250.50,
    available: 750.50,
    locked: 500.00,
  ...
  const handleClaim = async (betId: number) => {
    try {
      // INTEGRAÇÃO COM BACKEND: Descomentar quando claimWinnings estiver disponível
      // const result = await claimWinnings(betId);
      console.log(`Saque de aposta ${betId} iniciado...`);
      // TODO: Mostrar toast de sucesso com sonner
    } catch (error) {
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <h2 className="text-3xl font-bold text-white">
                    {userBalance.total.toFixed(2)} <span className="text-lg text-primary">{userBalance.currency}</span>
                  </h2>
                </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <h2 className="text-3xl font-bold text-primary">
                    {userBalance.available.toFixed(2)} <span className="text-lg">{userBalance.currency}</span>
                  </h2>
                </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <h2 className="text-3xl font-bold text-yellow-400">
                    {userBalance.locked.toFixed(2)} <span className="text-lg">{userBalance.currency}</span>
                  </h2>
                </div>
          ...
          <div className="lg:col-span-2">
            <div className="mb-8">
              ...
              <p className="text-muted-foreground font-mono text-sm">
                Total de {userBets.length} apostas
              </p>
            </div>
          ...
          <div className="lg:col-span-1">
            ...
            <Card className="bg-background border-border p-6 mb-6">
              ...
              <div className="space-y-4">
                <div className="border-b border-border pb-4">
                  ...
                  <p className="text-white font-bold text-2xl">33%</p>
                  <p className="text-muted-foreground text-xs font-mono mt-1">1 vitória em 3 apostas</p>
                </div>

                <div className="border-b border-border pb-4">
                  <p className="text-muted-foreground text-sm font-mono mb-1">Ganhos Totais</p>
                  <p className="text-green-400 font-bold text-2xl">+425 USDC</p>
                  <p className="text-muted-foreground text-xs font-mono mt-1">Lucro líquido</p>
                </div>

                <div>
                  <p className="text-muted-foreground text-sm font-mono mb-1">Apostas Ativas</p>
                  <p className="text-yellow-400 font-bold text-2xl">2</p>
            ...
            <Card className="bg-background border-border p-6">
              ...
              <div className="mt-6 p-4 bg-primary/10 border border-primary/30 rounded">
                <p className="text-primary text-xs font-mono leading-relaxed">
                  Dados mockados. Atualize useBettingContract() com funcoes reais do smart contract.
                </p>
              </div>
            </Card>
          </div>
