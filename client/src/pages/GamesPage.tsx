...
import { useFormingMarkets, useActiveMarkets, useSettledMarkets } from '@/hooks/useBackendMatches';
import type { BackendMatch } from '@/hooks/useBackendMatches';
...
function MatchListItem({ match, onBetClick }: { match: Match; onBetClick: (match: Match, outcome: 'home' | 'draw' | 'away') => void; }) {
    ...
    <Card className="p-4 bg-gray-900 border-gray-700 hover:border-primary/50 transition-colors cursor-pointer">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        ...
        <div className="md:col-span-3 flex justify-around text-center">
            <div onClick={() => onBetClick(match, 'home')} className="cursor-pointer p-2 rounded-md hover:bg-primary/10 transition-colors">
              <p className="text-xs text-white/70">Casa</p>
              <p className="text-lg font-bold text-green-400">{(match.dynamicOddsHome || 0).toFixed(2)}</p>
            </div>
            <div onClick={() => onBetClick(match, 'draw')} className="cursor-pointer p-2 rounded-md hover:bg-primary/10 transition-colors">
            ...
            <div onClick={() => onBetClick(match, 'away')} className="cursor-pointer p-2 rounded-md hover:bg-primary/10 transition-colors">
...
export default function GamesPage() {
  const [isBetDialogOpen, setBetDialogOpen] = useState(false);
  const [selectedMatch, setSelectedMatch] = useState<Match | null>(null);
  const [selectedOutcome, setSelectedOutcome] = useState<'home' | 'draw' | 'away' | null>(null);

  const handleBetClick = (match: Match, outcome: 'home' | 'draw' | 'away') => {
    // Adaptador: Converte o Match do useMatchData para o formato esperado pelo BetForm
    const adaptedMatch = {
      id: match.id,
      ...
      odds: {
        home: match.dynamicOddsHome,
        draw: match.dynamicOddsDraw,
        away: match.dynamicOddsAway,
      },
      poolSize: match.totalPoolAmount,
      timeRemaining: "", // Este campo não é usado no BetForm
      betCategories: [], // Este campo não é usado no BetForm
    };

    setSelectedMatch(adaptedMatch);
    setSelectedOutcome(outcome);
    setBetDialogOpen(true);
  };
  // Buscar mercados reais do backend
  const { markets: formingMarkets, isLoading: loadingForming } = useFormingMarkets();
  const { markets: activeMarkets, isLoading: loadingActive } = useActiveMarkets();
  const { markets: settledMarkets, isLoading: loadingSettled } = useSettledMarkets();
  
  const isLoading = loadingForming || loadingActive || loadingSettled;
  
  // Converter BackendMatch para Match
  const convertToMatch = (market: BackendMatch, status: 'open' | 'in_progress' | 'finished'): Match => ({
    ...
    status,
    dynamicOddsHome: 0, // Será calculado dinamicamente
    dynamicOddsDraw: 0,
    dynamicOddsAway: 0,
    realPoolHome: 0,
    realPoolDraw: 0,
  ...
  const matches: Match[] = [
    ...formingMarkets.map(m => convertToMatch(m, 'open')),
    ...activeMarkets.map(m => convertToMatch(m, 'in_progress')),
    ...settledMarkets.map(m => convertToMatch(m, 'finished'))
  ];
  const AVAILABLE_LEAGUES = Array.from(new Set(matches.map(m => m.league)));
  const isFeatured = (match: Match) => false; // Lógica de destaque precisa ser reavaliada
  ...
  return (
    <div className="container py-8">
      <Dialog open={isBetDialogOpen} onOpenChange={setBetDialogOpen}>
        <DialogContent className="cyber-dialog">
          {selectedMatch && selectedOutcome && (
            <BetForm 
              match={selectedMatch} 
              selectedOutcome={selectedOutcome} 
              selectedCategory="Jogo Todo" // Categoria padrão
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            {featuredMatches.map(match => (
              <MatchListItem key={match.id} match={match} onBetClick={handleBetClick} />
            ))}
          </div>
        </div>
      )}
