...
import { Search, List, LayoutGrid, Clock, TrendingUp, Filter, ChevronDown, ChevronUp } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        ...
        <div className="md:col-span-3 flex justify-around text-center">
          <div>
            ...
            <p className="text-lg font-bold text-green-400">{match.dynamicOddsHome.toFixed(2)}</p>
          </div>
          <div>
            <p className="text-xs text-white/70">Empate</p>
            <p className="text-lg font-bold text-yellow-400">{match.dynamicOddsDraw.toFixed(2)}</p>
          </div>
          <div>
            <p className="text-xs text-white/70">Fora</p>
            <p className="text-lg font-bold text-red-400">{match.dynamicOddsAway.toFixed(2)}</p>
          </div>
        </div>
        ...
        <div className="md:col-span-3 space-y-1">
          <div className="flex justify-between text-xs text-white/70">
            <span>Pool Casa: {match.realPoolHome.toFixed(2)} / {match.minPoolHome.toFixed(2)} USDC</span>
            <span className={isHomeValidated ? 'text-green-400' : 'text-red-400'}>{calculateProgress(match.realPoolHome, match.minPoolHome).toFixed(0)}%</span>
          </div>
          <div className="h-1 bg-gray-700 rounded-full">
            <div 
              className="h-1 rounded-full transition-all duration-500" 
              style={{ width: `${calculateProgress(match.realPoolHome, match.minPoolHome)}%`, backgroundColor: isHomeValidated ? '#4ade80' : '#f87171' }}
            ></div>
          </div>
          {/* Barra de Progresso Empate */}
          <div className="flex justify-between text-xs text-white/70">
            <span>Pool Empate: {match.realPoolDraw.toFixed(2)} / {match.minPoolDraw.toFixed(2)} USDC</span>
            <span className={isDrawValidated ? 'text-green-400' : 'text-red-400'}>{calculateProgress(match.realPoolDraw, match.minPoolDraw).toFixed(0)}%</span>
          </div>
          <div className="h-1 bg-gray-700 rounded-full">
            <div 
              className="h-1 rounded-full transition-all duration-500" 
              style={{ width: `${calculateProgress(match.realPoolDraw, match.minPoolDraw)}%`, backgroundColor: isDrawValidated ? '#4ade80' : '#f87171' }}
            ></div>
          </div>
          {/* Barra de Progresso Fora */}
          <div className="flex justify-between text-xs text-white/70">
            <span>Pool Fora: {match.realPoolAway.toFixed(2)} / {match.minPoolAway.toFixed(2)} USDC</span>
            <span className={isAwayValidated ? 'text-green-400' : 'text-red-400'}>{calculateProgress(match.realPoolAway, match.minPoolAway).toFixed(0)}%</span>
          </div>
          <div className="h-1 bg-gray-700 rounded-full">
            <div 
              className="h-1 rounded-full transition-all duration-500" 
              style={{ width: `${calculateProgress(match.realPoolAway, match.minPoolAway)}%`, backgroundColor: isAwayValidated ? '#4ade80' : '#f87171' }}
            ></div>
          </div>
        </div>
      </div>
